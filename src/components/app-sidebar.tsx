...
export function AppSidebar({ mobileOpen, onCloseMobile }: Props) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed left-0 top-0 z-[9999] h-screen w-64 bg-sidebar border-r border-sidebar-border",
          "transition-transform duration-200",
          "md:z-40 md:translate-x-0", // desktop always visible
          mobileOpen ? "translate-x-0" : "-translate-x-full" // mobile drawer
        )}
      >
            ...
            {navItems.map((item) => {
              ...
              return (
                <NavLink
                  ...
                  to={item.href}
                  onClick={() => {
                    // ✅ close drawer after click (mobile only)
          ...
          <div className="border-t border-sidebar-border p-4 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-sidebar-muted truncate">{me?.email ?? ""}</p>
              </div>
